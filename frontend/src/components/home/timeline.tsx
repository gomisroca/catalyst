...
export default function HomeTimeline(){
    ...
    useEffect(() => {
        ...
        function createTimeline(follows: User[]): void {
            ...
            const filteredTimeline: InteractionOrProjectOrBranchOrPost[] = sortedTimeline.filter(obj => 
                obj.type == 'LIKE' || 
                obj.type == 'SHARE' || 
                obj.content || 
                (obj.projectId && 
                    (obj.permissions?.private == false || 
                    user && (obj.author?.id == user.id || 
                    obj.permissions?.allowedUsers.includes(user.id)) ) ) || 
                (obj.name && 
                    !obj.projectId && 
                    (obj.permissions?.private == false || 
                    user && (obj.author?.id == user.id || 
            ...
            (obj.type == 'LIKE' || obj.type == 'SHARE') ?
                <Card key={obj.id} className="px-0 pt-2 bg-secondary/20 border-none">
                    ...
                    <CardDescription className="flex items-center gap-1 mx-6">
                        <Star size={"15px"} />
                        {obj.author && (obj.author.nickname || obj.author.username)} liked a {obj.branchId ? 'branch' : 'post'}
                    </CardDescription>
                    ...
                    <CardDescription className="flex items-center gap-1 mx-6">
                        <Forward size={"15px"} />
                        {obj.author && (obj.author.nickname || obj.author.username)} shared a {obj.branchId ? 'branch' : 'post'}
                    </CardDescription>
                    <CardContent className="p-2">
                        {obj.branchId && obj.branch ?
                            <TimelineBranchCard branch={obj.branch} />
