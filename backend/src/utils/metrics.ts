...
function popularityPerBranch(branches) {
  ...
  return branches.map((branch) => {
    const branchPostInteractions = branch.posts.reduce((postTotal, post) => {
      const postInteractionsLastWeek = post.interactions.filter(
        (interaction) => new Date(interaction.createdAt) > oneWeekAgo
      );
      const positiveInteractions = postInteractionsLastWeek.filter(
        (interaction) => interaction.type !== 'REPORT' && interaction.type !== 'HIDE'
      );
      const negativeInteractions = postInteractionsLastWeek.filter(
        (interaction) => interaction.type == 'REPORT' || interaction.type == 'HIDE'
      );
    ...
    const branchInteractions = branch.interactions.filter(
      (interaction) => new Date(interaction.createdAt) > oneWeekAgo
    );
    const positiveInteractions = branchInteractions.filter(
      (interaction) => interaction.type !== 'REPORT' && interaction.type !== 'HIDE'
    );
    const negativeInteractions = branchInteractions.filter(
      (interaction) => interaction.type == 'REPORT' || interaction.type == 'HIDE'
    );
    const interactionsLastWeek = positiveInteractions.length - negativeInteractions.length;
...
export async function setMetrics(projects) {
  ...
  for (const project of projects) {
    ...
    for (const branchPopularity of popularity) {
      branchesPopularity.push({
        branchId: branchPopularity.branchId,
