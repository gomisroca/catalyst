...
export default function UserSettingsForm({ user, modal = false }: { user: User; modal?: boolean }) {
  ...
  const formAction = async (formData: FormData) => {
    try {
      ...
      const action = await updateUserSettings({
      ...
      setMessage({ content: toErrorMessage(error, 'Failed to update user settings'), type: 'error' });
    }
  };
      ...
      <section className="flex w-full flex-col gap-2">
        ...
        {user.image && (
          <div className="mx-auto flex flex-col text-center">
            ...
            <Image className="rounded-lg" src={user.image} height={200} width={200} alt="Avatar" />
          </div>
        )}
        <input
